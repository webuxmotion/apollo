...
> **Note:** Because breaking changes are detected by analyzing recent operations, your GraphQL server _must_ be [pushing analytics to Apollo Studio](/setup-analytics/) for schema checks to work. **If there are no operation metrics to compare against, all potentially dangerous schema changes are labeled `FAIL`.**
...
You can also rerun checks from inside Studio. When you rerun a check, the latest check configuration will be applied.
The new check will use an updated time window for operations — in addition, any changes made to excluded / included clients, checked variants, and any operations already marked safe / ignored will be incorporated when rerunning.

> **Note:** If you've [integrated schema checks with your GitHub PRs](#integrating-with-github), a rerun of the check will also update the status of the check in Github.
